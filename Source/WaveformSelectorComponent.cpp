/*
  ==============================================================================
    WaveformSelectorComponent.cpp (SPLENTA V18.6 - 20251216.09)
    ...
    : apvts(apvts)
{
    ...
    if (param != nullptr)
        selectedIndex = juce::roundToInt(param->load());
}
...
void WaveformSelectorComponent::paint(juce::Graphics& g)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        // Background (selected = bright, unselected = dim)
        bool isSelected = (i == selectedIndex);
        juce::Colour bgColor = isSelected
            ? colors[i].withAlpha(0.3f)
        ...
        // Draw waveform icon
        auto iconBounds = segmentBounds.reduced(12.0f);
        switch (i)
        {
...
void WaveformSelectorComponent::mouseDown(const juce::MouseEvent& event)
{
    ...
    float segmentWidth = getWidth() / 3.0f;
    int clickedIndex = juce::jlimit(0, 2, (int)(clickX / segmentWidth));

    // Update parameter
    auto* param = apvts.getParameter("SHAPE");
    if (param != nullptr)
    {
        param->setValueNotifyingHost(param->convertTo0to1((float)(clickedIndex + 1)));
